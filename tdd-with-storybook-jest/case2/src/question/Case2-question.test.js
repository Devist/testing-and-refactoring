...
// 해당 테스트 제거
// 이유는 , 새로운 테스트 케이스를, 이미지 컴포넌트를 별도로 만들며 거기에 만들어 두었기 때문
...
it('[question] 제품 설명 - 1986년에 만들어진 빈티지 시계입니다. 어느곳에나 잘 어울립니다. 인테리어로 분위기 Up 하세요', async () => {
  ...
  expect(descriptionContent.childNodes[1].textContent.trim()).toBe(
    '1986년에 만들어진 빈티지 시계입니다. 어느곳에나 잘 어울립니다. 인테리어로 분위기 Up 하세요'
  )
})
...
it('[question] 결제 금액은 - 수량 * 제품 가격', async () => {
  ...
  expect(sellerContent.childNodes[1].textContent.trim()).toBe(
    payment.toString()
  )
})
