...
it("[solution] 주문자 정보를 불러와야 한다.", async () => {
  const { container } = render(<Root {...vintage_clock} coupons={coupons} userPoint={0} />);

  const sellerContent = screen.getByText("판매자");
  fireEvent.click(sellerContent);
